...
- Xiaoduiyou Hermes 插件会维护 Hermes 可读的 channel directory 兼容条目：`send_message(action="list")` 应能看到 `xiaoduiyou:主对话`、`xiaoduiyou:<频道名>`，家庭群频道类型应为 `group`。如果列表里只有旧 `sess_...` 或旧 `悬浮会话`，重新执行安装/更新提示词并重启对应 Hermes profile。
...
更新后确认 `send_message(action="list")` 能看到 `xiaoduiyou:主对话` 和当前小队友频道名；如果只看到旧 `sess_...`，重新执行安装脚本并重启对应 Hermes profile。
