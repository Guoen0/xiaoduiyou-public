...
- When swapping/reverting one carousel image, update all three surfaces in one pass: `publish_notes.xiaohongshu.images[index]`, the process document's `### 最终配图` `image_grid`, and the history grid/captions so the replaced version is still visible for comparison. If a user says an older candidate is better, prefer reverting to that exact URL unless they explicitly ask for a new generation.
...
- If the user reports "发布页还是没有" or that platform images did not change, immediately read `publish_notes.xiaohongshu` with `view=field` and compare the live `images` array against the intended URLs. If the field is stale, patch `fields.publish_notes.xiaohongshu` directly with `command="patch_fields"` and the complete Xiaohongshu note shape; do not rely on a broad `overwrite` of `block_json` + fields to update the publish tab. Document updates may be queued for final callback, so treat tool success as queued until a later read/screen summary confirms the visible field.
- Avoid mixing a direct `patch_fields` call with a subsequent broad `overwrite` built from stale document state in the same turn. A later overwrite can silently revert the newer `publish_notes.images` or `image_plan` back to old URLs. When both process-doc blocks and publish fields must change, build one final payload from the latest intended state, or update blocks first and finish with a final `patch_fields` for `publish_notes.xiaohongshu`.
