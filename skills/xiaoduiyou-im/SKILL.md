...
8a. **Do not turn a historical preference clue into a repeated gift/product recommendation.** Before recommending, separate explicit likes, inferred aesthetic signals, products/actions already used, and unknowns. Anything already done belongs on an exclusion list unless the user explicitly asks to repeat it.
8b. **Never describe an imagined product archetype as though a matching SKU has been found.** Search and verify the exact listing first. If the market only offers partial matches (for example, plated silver instead of solid 18K gold), say that plainly and recommend from verified candidates rather than inventing precise-sounding series names.
8c. **For jewelry sizing, never convert from a vague remembered number or an unrelated size.** Inspect the live SKU's available options and declared sizing standard, use inner diameter in millimetres as the cross-system reference, measure a ring from the intended finger, and account for wide/multi-band styles fitting tighter. If the page shows only unexplained size numbers, ask the seller to confirm the system and finished inner diameter. Detailed checklist: `references/product-question-workflow.md`.
