...
12. Keep private family context out of skill files. In a local Hermes environment, treat `${HERMES_HOME:-$HOME/.hermes}/private/xiaoduiyou-family-care-preferences.md` as the only durable private-family memory file: read it before using family-specific names, Feishu IDs, caregiver mappings, childcare preferences, or durable care-history facts; create or update it when the user gives durable private context; and keep reusable diary behavior in this skill. The file lives outside `skills/` and `plugins/`, so skill/plugin upgrades should not overwrite it.
...
| `上传照片到成长日记` | First-class connector asset/tool support when available + `references/image-upload-contract.md` | Upload local/generated photos first; public fetchable HTTPS photo URLs can be written directly. Do not discover tokens manually. |
...
| Recommend new foods / compare eaten vs. untried | `references/dietary-recommendation-safety.md` + Growth Diary + `xiaoduiyou_child_get` | Cross-check explicit food history against live allergy/avoidance profile before recommending. |
...
- Recover safely from interrupted turns instead of blindly replaying writes:
  - If interruption happened before `xiaoduiyou_growth_diary_patch` was called, the event is still pending. On the next turn, read the target date/type to confirm absence, then create it; when a new caregiver event also arrived, both records may be created in one PATCH if their times are independently known.
  - If interruption happened during or after PATCH, treat commit status as unknown. Query the exact date/type/time/content or quantity first; retry only when the intended event is absent, otherwise acknowledge the existing row. This avoids duplicate milk, meal, and stool records.
  - Preserve the original pending message's timestamp. Never replace it with the recovery turn time; use the original inbound timestamp/session trace, or the row's `created_at` only when it is the sole connector-visible trace.
- For high-frequency care-event verification, use a filter that distinguishes the new row from same-day repeats. Quantity alone may return several milk rows; confirm the intended `occurred_at`. For meals, query one distinctive literal ingredient or an explicit clock text; for stool, query the explicit clock text when available. Do not assume a space-separated multi-ingredient query has OR semantics: it may return zero even when the row exists. If a compound query misses, retry with one distinctive ingredient before concluding the write failed.
- When the caregiver sends an incomplete/ASR-fragment message and immediately follows with a complete version of the same current event, treat the pair as one real-world event. Dedupe before writing, use the complete wording for title/content, and preserve both fragments in `original_message` when useful for traceability.
- If the user says `现在` but the connector input does not expose the inbound `created_at`, do not use the latest visible timeline event as the event time—the screen may be hours stale. Prefer an exact gateway/session timestamp when available. If no exact timestamp is available before creation and the event must be recorded immediately, use the closest defensible provisional time only to satisfy the required datetime, then immediately read the newly created exact row, take its connector `created_at` as the sole timestamp trace, convert it to the home-local datetime, patch `occurred_at`, and verify again. Keep this repair within the same turn and never leave the provisional time as the final record.
- For edits to an existing record, use `updates` as **one item per field**: `{table_id, record_id, field_id, value}`. Never send `{record_id, values: {...}}` inside `updates`; `values` is only valid for new `records` and can fail with `COLUMN_NOT_FOUND`. Concrete pattern:
  ```json
  {
    "updates": [
      {"table_id":"tbl_growth_events","record_id":"agent_...","field_id":"occurred_at","value":"2026-01-15 08:15:00"},
      {"table_id":"tbl_growth_events","record_id":"agent_...","field_id":"content","value":"8:15 喝奶 200ml。"},
      {"table_id":"tbl_growth_events","record_id":"agent_...","field_id":"original_message","value":"早上的奶是 8:15 喝的"}
    ]
  }
  ```
  Afterward, verify with a filtered GET. The PATCH `changed_records` summary is a concise projection and may display unchanged fields as blank; do not misread those blanks as data loss.
- When the caregiver supplements a referenced meal with another ingredient, treat it as an edit to that real-world meal, not a new meal. Query the same-day meal by a distinctive existing ingredient, preserve its `occurred_at`, then update `title`, `content`, and `original_message` as separate field-level updates. Verify using the newly added ingredient.
- When correcting a recently logged no-time event, identify the exact record by date/event/title/source first, then set `occurred_at` to the original Xiaoduiyou message timestamp (or the existing record `created_at` converted to local event time if that is the only connector-visible trace), not the page date and not the Agent runtime clock. If the caregiver corrects the same event more than once, keep updating that same `record_id` rather than creating another row, and verify the latest explicit time after every correction.
- When one inbound turn contains several independently timed care events, read each relevant event type for dedupe, then create all absent events together in one `records` array. Preserve each event's own time: explicit clock time stays with that event, while `现在`/time-less events use their own Xiaoduiyou message timestamp. Never apply one event's time to the others. After the combined PATCH, verify each event separately with a distinguishing filter and confirm the intended `occurred_at`.
- A single inbound turn may contain repeated Xiaoduiyou sender/screen/platform envelopes around multiple caregiver messages. Treat envelope metadata and repeated visible summaries as context, not diary events; extract each actual caregiver event exactly once. Do not drop the first event just because a second envelope follows it, and do not duplicate an event because the same screen summary appears twice.
- After PATCH plus a filtered GET returns the intended row with the intended values, the operation is verified. Give the concise success confirmation; do not replace it with a generic `Operation interrupted`/timeout message. If interruption occurs before verification, query exact date/type/time/content first and only retry the write when the row is absent.
...
- Current screen/page date and visible timeline summary are UI context only and may lag behind successful writes. The user may be viewing a different date/page, or the screen may still say `当天暂无记录` after a connector write. Never let visible UI override explicit user time/date or Xiaoduiyou message-send time, and never use the screen summary as proof that a row is absent; query live records with `xiaoduiyou_growth_diary_get` for dedupe and verification.
- Resolve relational meal shorthand such as `吃上一顿剩下的饭` against live same-day records when unambiguous. Keep the shorthand in `original_message`/title while adding known ingredients to content for traceability. If several earlier meals are plausible, do not invent which leftovers were eaten: record the generic wording, or ask only when the distinction is safety-critical.
...
- Normalize stool-character shorthand into structured risk/tags without overreacting. A single loose-stool report should remain a stool event, use `risk: "need_watch"`, and add tag `偏稀`; use `水样` only when the caregiver explicitly says watery/water-like. Keep the confirmation factual and brief. Escalate beyond `need_watch` only for repetition or red flags such as blood/black stool, poor energy, low urine, dry lips, fever, or vomiting.
- Classify non-milk drinks by substance, not by the verb `喝`: juice, yogurt drinks, soup, and similar intake are food/meals unless the live schema exposes a more specific drink/water option. Preserve the caregiver's wording and do not invent volume when none was given.
