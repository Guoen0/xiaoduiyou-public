...
def _tool_update_document(args: Dict[str, Any], **_: Any) -> str:
    ...
    return json.dumps({
        ...
        "queued": False,
        "applied": result.get("applied", mutation.get("applied", True)) if isinstance(result, dict) else True,
        "persisted": result.get("persisted", mutation.get("persisted", True)) if isinstance(result, dict) else True,
