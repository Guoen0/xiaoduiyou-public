...
def _require_requests():
    try:
        ...
        # 在受限环境（如缺少 pip、PEP 668 系统）中，尝试用 urllib 回退
        print("Error: requests not installed. Run: pip install requests", file=sys.stderr)
        print("Note: On PEP 668 systems or where pip is unavailable, use a venv: python3 -m venv .venv && source .venv/bin/activate && pip install requests", file=sys.stderr)
        sys.exit(1)
    return requests
